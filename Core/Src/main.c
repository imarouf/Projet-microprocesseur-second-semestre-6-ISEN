...
/* USER CODE BEGIN 0 */
int step = 0;
IKS01A3_MOTION_SENSOR_Axes_t dataXYZ;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  int init = 0;
   ...
   if(IKS01A3_MOTION_SENSOR_Init(IKS01A3_LIS2DW12_0, MOTION_ACCELERO) != BSP_ERROR_NONE){
	  printf("error init accelerometer\n\r");
   }
   else{
   ...
   if(IKS01A3_ENV_SENSOR_Init(INSTANCE, ENV_HUMIDITY)!= BSP_ERROR_NONE){
 	  printf("error init humidity\n\r");
   }
   else{
 	  printf("humidity init\n\r");
 	  init++;
   }
  ...
  /* USER CODE BEGIN WHILE */
   while (1)
   {
    ...
    /* USER CODE BEGIN 3 */
       ...
       HAL_ADC_Stop(&hadc);
       uint8_t brightness = (analogValue * 15) / 4095;
...
void AfficheHum(void)
{
    ...
    if(IKS01A3_ENV_SENSOR_GetValue(INSTANCE, ENV_HUMIDITY, &humidity)==BSP_ERROR_NONE)
    {
    	...
    	sprintf(hum_str, "%.0f", humidity);
    		 for (int i = 0; i < strlen(hum_str)+1; i++)
    		 {
    			 MAX7219_DisplayChar(i+1,hum_str[i]);
    		 }
    		 MAX7219_DisplayChar(4,'H');
        printf("humidite : %f\r\n", humidity);
    }
}
...
void AfficheStep(void)
{
	if(IKS01A3_MOTION_SENSOR_GetAxes(IKS01A3_LIS2DW12_0, MOTION_ACCELERO, &dataXYZ) == BSP_ERROR_NONE){
		printf("Acc: X=%ld, Y=%ld, Z=%ld\n\r", dataXYZ.x, dataXYZ.y, dataXYZ.z);
		float x_g = dataXYZ.x / 1000.0f;
		float y_g = dataXYZ.y / 1000.0f;
		float z_g = dataXYZ.z / 1000.0f;
		detect_steps(x_g, y_g, z_g);

	}
	char step_str[20];
    sprintf(step_str, "%.0f", (float)step);
    for (int i = 0; i < strlen(step_str)+1; i++)
    {
    	MAX7219_DisplayChar(i+1,step_str[i]);
    }
    MAX7219_DisplayChar(4,'P');
}
